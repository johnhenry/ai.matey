const methodPrompt = (prop, args) => {
  const list = args.length
    ? ` with the following arguments:\n${args
        .map((arg) => ` - ${JSON.stringify(arg)}`.trim())
        .join("\n")}`
    : "";
  return `You are responding to a function-like prompt named "${prop.replaceAll(
    "_",
    " "
  )}"${
    list ? list : "."
...
class Session {
    ...
    this.#$ = new Proxy(this, {
      get(target, prop) {
        return async function (...args) {
          let result;
          result = await this.prompt.call(target, methodPrompt(prop, args));
          try {
  ...
  _addToHistory(userMessage, assistantMessage) {
    ...
    if (
      ...
      this.#conversationHistory.length > this.#maxHistorySize * 2
    ) {
      // Before removing messages, subtract their tokens
      const removedMessages = this.#conversationHistory.slice(
        0,
