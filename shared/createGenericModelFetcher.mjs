  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error(
          `Failed to parse error response from ${providerName} API:`,
          e
        );
      }
    ...
    if (!Array.isArray(modelsList)) {
      console.warn(
        `${providerName} API response format unexpected: model list not found or not an array at path '${
    ...
    if (typeof modelIdExtractor !== "function") {
      ...
      throw new Error(
        `${providerName} modelIdExtractor is not properly configured.`
      );
    }
