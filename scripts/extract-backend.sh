#!/bin/bash
# Script to extract a backend adapter into its own package
...
PACKAGE_DIR="packages/backend-$ADAPTER"
PACKAGE_NAME="ai.matey.backend.$ADAPTER"
...
# Copy adapter file
cp "src/adapters/backend/$ADAPTER.ts" "$PACKAGE_DIR/src/index.ts"
...
cat > "$PACKAGE_DIR/package.json" << EOF
{
  "name": "$PACKAGE_NAME",
  "version": "1.0.0",
  "description": "${ADAPTER^} backend adapter for AI Matey",
  "type": "module",
  ...
  "keywords": ["ai", "llm", "$ADAPTER", "adapter", "ai-matey"],
  "author": "AI Matey",
  "license": "MIT",
  "repository": {
    "type": "git",
