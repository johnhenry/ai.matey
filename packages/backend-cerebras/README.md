...
Backend adapter for Cerebras Inference
...
const backend = new CerebrasBackendAdapter({
...
The main adapter class for Cerebras.

#### Constructor

```typescript
new CerebrasBackendAdapter(config: CerebrasBackendAdapterConfig)
```
...
const response = await backend.execute({
  messages: [{ role: 'user', content: 'Hello!' }],
  parameters: { model: 'cerebras/Cerebras-GPT-13B' },
