...
const response = await bridge.chat({
  model: 'Various HuggingFace models',
...
The main adapter class for Hugging Face.

#### Constructor

```typescript
new HuggingFaceBackendAdapter(config: HuggingFaceBackendAdapterConfig)
```
...
const response = await backend.execute({
  messages: [{ role: 'user', content: 'Hello!' }],
  parameters: { model: 'Various HuggingFace models' },
