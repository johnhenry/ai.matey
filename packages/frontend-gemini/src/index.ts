...
import type { GeminiRequest, GeminiResponse, GeminiContent } from 'ai.matey.backend.gemini';
...
export class GeminiFrontendAdapter implements FrontendAdapter<GeminiRequest, GeminiResponse> {
  ...
  async toIR(request: GeminiRequest): Promise<IRChatRequest> {
    const messages: IRMessage[] = request.contents.map((c: GeminiContent) => ({ role: c.role === 'model' ? 'assistant' : 'user', content: c.parts.map((p: { text: string } | { inlineData: { mimeType: string; data: string } }) => ('text' in p ? p.text : '')).join('') }));
    if (request.systemInstruction) messages.unshift({ role: 'system', content: request.systemInstruction.parts.map((p: { text: string }) => p.text).join('') });
