...
Backend adapter for Groq API (fast inference)
...
The main adapter class for Groq.

#### Constructor

```typescript
new GroqBackendAdapter(config: GroqBackendAdapterConfig)
```
...
const response = await backend.execute({
  messages: [{ role: 'user', content: 'Hello!' }],
  parameters: { model: 'llama-3.1-70b' },
