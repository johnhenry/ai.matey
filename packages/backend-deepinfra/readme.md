...
Backend adapter for DeepInfra API
...
```typescript
import { DeepInfraBackendAdapter } from 'ai.matey.backend.deepinfra';
import { Bridge } from 'ai.matey.core';
import { OpenAIFrontendAdapter } from 'ai.matey.frontend.openai';

// Create the backend adapter
const backend = new DeepInfraBackendAdapter({
...
The main adapter class for DeepInfra.

#### Constructor

```typescript
new DeepInfraBackendAdapter(config: DeepInfraBackendAdapterConfig)
```
...
const response = await backend.execute({
  messages: [{ role: 'user', content: 'Hello!' }],
  parameters: { model: 'meta-llama/Llama-2-70b-chat-hf' },
