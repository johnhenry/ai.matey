...
Chrome AI API wrapper for any backend
...
const client = ChromeAILanguageModel(
...
```typescript
ChromeAILanguageModel(backend: BackendAdapter, config?: WrapperConfig): Client
```
...
import { ChromeAILanguageModel } from 'ai.matey.wrapper.chrome-ai';
import { AnthropicBackendAdapter } from 'ai.matey.backend.anthropic';
const client = ChromeAILanguageModel(new AnthropicBackendAdapter({ apiKey: '...' }));
