...
Backend adapter for Replicate API
...
```typescript
import { ReplicateBackendAdapter } from 'ai.matey.backend.replicate';
...
The main adapter class for Replicate.

#### Constructor

```typescript
new ReplicateBackendAdapter(config: ReplicateBackendAdapterConfig)
```
...
const response = await backend.execute({
  messages: [{ role: 'user', content: 'Hello!' }],
  parameters: { model: 'meta/llama-2-70b-chat' },
